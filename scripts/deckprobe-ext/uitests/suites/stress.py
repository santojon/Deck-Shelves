...
What it measures
----------------
- Home render time with 30 regular + 17 smart shelves at limit=50 each
  (includes composite source, multi-key sort, decoration cards, the
  heuristic templates, the runtime-aware templates, plus the per-shelf
  logo / icon / description overlay paths and the master-switch toggles).
...
- Per-shelf logo / hero overlay paint cost on focus traversal.
- Composite intersection resolve under the 15 s race ceiling.
- Manual sort + dedupe + hiddenAppIds overshoot interactions.
- Full-page shelf promoted layout (forceExpanded + pinScrollTop).

Thresholds
----------
MOUNT_WARN_MS     = 18000  — cold render with 30 shelves / 50 cards each.
NAV_FRAME_MAX_MS  = 600    — worst single frame.
NAV_FRAME_AVG_MS  = 50     — average frame during navigation.
ENTER_EXIT_MS     = 25000  — round-trip incl. store page load.
"""
...
_STRESS_MIN_SHELVES = 10  # stress fixture has 30 regular + 17 smart = 47 total
...
@s.test("logo / icon / description overlay does not blow the frame budget on focus traversal")
def _(ctx) -> None:
    """Stress shelf 20 enables per-shelf logo + description; shelf 21
    explicitly disables them. Sample frames while scrubbing horizontally
    across both shelves: the overlay re-derivation, image-cache lookup,
    and description-cache lookup must stay within the navigation budget."""
    _require_stress_fixture(ctx)
    ctx.navigate("/library/home", settle_ms=800)
    n = _wait_for_shelves(ctx, min_count=_STRESS_MIN_SHELVES, timeout_s=15.0)
    assert n >= _STRESS_MIN_SHELVES, f"only {n} shelves mounted"
    overlay_count = ctx.eval("""
(function(){
    return document.querySelectorAll('.ds-shelf-logo-overlay').length;
})()
""") or 0
    if overlay_count == 0:
        from deckprobe.uitests.lib.runner import SkipTest
        raise SkipTest("no logo overlays mounted on this device")
    ctx.eval(_INSTALL_COLLECTOR)
    ctx.eval(_CLEAR_ERRORS)
    # Walk right 12 times then left 12 — exercises focused-card change
    # (logo redraw + description re-resolve via appDescriptionsCache).
    for _ in range(12):
        _key(ctx, "ArrowRight", pause_ms=60)
    for _ in range(12):
        _key(ctx, "ArrowLeft", pause_ms=60)
    stats = _sample_frames(ctx, steps=30)
    print(f"  → logo overlay traversal: overlays={overlay_count} max={stats['max']}ms avg={stats['avg']}ms")
    assert stats["max"] < NAV_FRAME_MAX_MS, f"overlay traversal frame {stats['max']}ms > {NAV_FRAME_MAX_MS}ms"
    assert stats["avg"] < NAV_FRAME_AVG_MS * 1.5, f"overlay traversal avg {stats['avg']}ms > {NAV_FRAME_AVG_MS * 1.5}ms"
    _assert_no_errors(ctx, "logo overlay traversal")


@s.test("composite intersection resolves under the 15 s race ceiling")
def _(ctx) -> None:
    """Shelf 25 is a composite intersection of two children; the resolver
    runs them in parallel under a 15 s race so a single hung child can't
    stall the row. Verify the intersection shelf does mount inside the
    mount budget (i.e. neither child timed out unexpectedly)."""
    _require_stress_fixture(ctx)
    ctx.navigate("/library/home", settle_ms=800)
    target = ctx.eval("""
(async function(){
    ...
    while (Date.now() < deadline) {
        const el = document.querySelector('.ds-shelf[data-shelfid="qa_st_25"]');
        if (el) {
            const cards = el.querySelectorAll('.ds-card[data-appid]').length;
            return { mounted: true, cards };
        }
        await new Promise(r => setTimeout(r, 200));
    }
    return { mounted: false, cards: 0 };
})()
""", timeout=20)
    if not target or not target.get("mounted"):
        from deckprobe.uitests.lib.runner import SkipTest
        raise SkipTest("composite intersection shelf qa_st_25 not present")
    print(f"  → composite intersection: cards={target.get('cards', 0)}")
    _assert_no_errors(ctx, "composite intersection")


@s.test("full-page shelf promotes without overlapping cards")
def _(ctx) -> None:
    """Shelf 22 is fullPageShelf + heroEnabled. The logo and hero overlay
    must compose without overflowing the card row (the post-2.4 regression
    where the absolute logo overlapped cards landed on this exact path)."""
    _require_stress_fixture(ctx)
    ctx.navigate("/library/home", settle_ms=1000)
    layout = ctx.eval("""
(async function(){
    const deadline = Date.now() + 12000;
    while (Date.now() < deadline) {
        const el = document.querySelector('.ds-shelf[data-shelfid="qa_st_22"]');
        if (el) {
            const cs = getComputedStyle(el);
            const overlay = el.querySelector('.ds-shelf-logo-overlay');
            const overlayTop = overlay ? parseFloat(getComputedStyle(overlay).top) : null;
            const firstCard = el.querySelector('.ds-card');
            const cardTop = firstCard ? firstCard.getBoundingClientRect().top : null;
            const overlayBottom = overlay ? overlay.getBoundingClientRect().bottom : null;
            return {
                mounted: true,
                ...
                cardTop,
                overlayBottom,
                clears: overlay && firstCard
                    ? overlayBottom <= cardTop + 4
                    : true,
            };
        }
        await new Promise(r => setTimeout(r, 200));
...
""", timeout=15)
    if not layout or not layout.get("mounted"):
        from deckprobe.uitests.lib.runner import SkipTest
        raise SkipTest("full-page shelf qa_st_22 not present")
    print(f"  → full-page shelf: paddingTop={layout.get('paddingTop')} minHeight={layout.get('minHeight')} clears={layout.get('clears')}")
    assert layout.get("clears") is True, (
        f"full-page shelf logo overlay overlaps card row: "
        f"overlayBottom={layout.get('overlayBottom')} cardTop={layout.get('cardTop')}"
    )
    _assert_no_errors(ctx, "full-page shelf layout")


@s.test("master-switch toggles propagate without per-shelf override leakage")
def _(ctx) -> None:
    """Shelves 20 and 21 set per-shelf `enableLogo` true / false respectively.
    The global master switch should win in both directions: when global is
    off, neither shelf shows a logo overlay. We probe BOTH the live shared
    global AND the localStorage settings cache — the shared global can be
    `undefined` immediately after mount before the settings store has
    emitted, so a single source produces false-negatives. Skips when the
    two sources disagree (transient state) or when neither has a value."""
    _require_stress_fixture(ctx)
    ctx.navigate("/library/home", settle_ms=800)
    snapshot = ctx.eval("""
(function(){
    const live = window.__DECK_SHELVES_SHARED_SETTINGS__ || null;
    let cached = null;
    try {
        const raw = localStorage.getItem('deck-shelves-settings-cache-v3');
        if (raw) cached = JSON.parse(raw);
    } catch {}
    const liveGlobal = live && typeof live.globalEnableLogo === 'boolean'
        ? live.globalEnableLogo : null;
    const cacheGlobal = cached && typeof cached.globalEnableLogo === 'boolean'
        ? cached.globalEnableLogo : null;
    const overlays = document.querySelectorAll('.ds-shelf-logo-overlay').length;
    return { liveGlobal, cacheGlobal, overlays };
})()
""") or {}
    live_g = snapshot.get("liveGlobal")
    cache_g = snapshot.get("cacheGlobal")
    n = snapshot.get("overlays", 0)
    # Pick the canonical value: prefer the live store; fall back to the
    # cache; require both sources to agree before asserting.
    from deckprobe.uitests.lib.runner import SkipTest
    if live_g is None and cache_g is None:
        raise SkipTest("settings global not exposed on this build (live + cache both empty)")
    if live_g is not None and cache_g is not None and live_g != cache_g:
        raise SkipTest(f"settings global in transient state: live={live_g} cache={cache_g}")
    g = live_g if live_g is not None else cache_g
    if g is False:
        assert n == 0, (
            f"global logo OFF but {n} overlay(s) rendered — per-shelf=true leaked through "
            f"(live={live_g} cache={cache_g})"
        )
    else:
        # When global is ON we expect at least one overlay (per-shelf=true
