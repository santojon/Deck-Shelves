...
def ws_connect(path):
    s = socket.socket(); s.settimeout(15); s.connect(("127.0.0.1", 8081))
...
s0 = socket.socket()
s0.settimeout(5)
s0.connect(("127.0.0.1", 8081))
s0.sendall(b"GET /json HTTP/1.1\r\nHost: 127.0.0.1:8081\r\n\r\n")
r = b""
while True:
    try:
        c = s0.recv(4096)
...
if not shared:
    ...
    sys.exit(1)
ws_path = shared[0]["webSocketDebuggerUrl"].split("127.0.0.1:8081", 1)[1]
