"""
Minimal CDP wrapper used by the modular screenshot pipeline.

Exposes a `Session` that opens a WebSocket against a single target and
sends Chrome DevTools Protocol calls. Designed to be a leaner peer of
the monolithic `screenshot.py` — covers only the methods the navigation
and capture helpers need.
"""
...
@dataclass
class Session:
    """Holds a single CDP WebSocket and provides `call(method, params)`."""
