...
- **Filtro de Remote Play: nova opção "Apenas neste dispositivo"**, espelhando a opção já existente "Apenas em outro dispositivo" — para uma prateleira só com jogos instalados aqui e em nenhum outro lugar.
- **As configurações ficaram mais resistentes a perda de dados.** Um campo que sua versão ainda não reconhecia — de uma versão mais nova, outro dispositivo, ou após um downgrade — costumava ser descartado silenciosamente ao salvar; agora esses campos passam intactos. Um arquivo de configurações corrompido podia destruir seu próprio backup no salvamento seguinte, perdendo os dois; agora ele é colocado em quarentena e recuperado automaticamente, com um aviso único se isso acontecer. Uma atualização feita pelo host independente também podia deixar uma cópia antiga do plugin rodando junto com a nova — corrigido.
- **O "Exportar tudo" agora exporta tudo, de verdade.** Cerca de 60 configurações — Modo Showcase, a proteção de tela, opções de desenvolvedor e outras — ficavam silenciosamente de fora do Exportar/Importar/Resetar mesmo quando selecionadas; agora toda configuração está coberta. As exportações também passam a carregar um selo de versão para compatibilidade futura, mas arquivos de exportação antigos continuam importando normalmente, e importar agora tira um snapshot de segurança das suas configurações atuais primeiro.
- **Configurações de desenvolvedor/depuração e a alternância "aba própria no QAM" agora são só do dispositivo.** Trocar de perfil, uma troca automática de perfil, um reset de fábrica ou uma sincronização entre dispositivos podiam alterá-las — agora não alteram mais, já que são específicas desta instalação, não algo que um perfil ou outro dispositivo deveria carregar.
- **Menos checagens de atualização redundantes ao rodar pelo host independente.** Ele já verifica atualizações sozinho e avisa por conta própria, então o plugin não checa mais nem mostra seu próprio banner ali — só ao rodar por um plugin loader, onde nada mais está checando.
- **Corrigido: a substituição de horário por dia de uma prateleira inteligente (ex.: Horas Vagas aberta o dia todo no fim de semana em vez das janelas curtas de sempre) virava uma janela de horário comum, todo dia, assim que você salvava**, e a chave "permitir substituições por dia" parecia ter se desligado sozinha. Salvar agora preserva a restrição de dia.
