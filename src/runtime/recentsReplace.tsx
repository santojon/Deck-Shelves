...
function scheduleResolve(shelf: any) {
  ...
  resolvePromise = platform
    ...
    .then((ids: number[]) => {
      ...
      // Native recents renderer only works for appids in the local app store.
      // If none of the resolved appids are owned (typical for online sources
      // like wishlist / free-now), skip to the next candidate.
      if (known.length === 0) {
        const candidates = visibleCandidateShelves();
