...
export function ShelfSideNav() {
  ...
  useEffect(() => {
    ...
    const tryOpen = () => {
      ...
      const last = lastFirstCardRef.current;
      if (last) {
        shelfId = last.shelfId;
        appid = last.appid;
      } else if (visibleRegular.length > 0) {
        shelfId = visibleRegular[0].id;
      } else if (visibleSmart.length > 0) {
        shelfId = visibleSmart[0].id;
      }
      if (!shelfId) return;
...
}>(function ShelfButton({ label, active, focused, disabled, icon, onActivate, onFocusChange, hoverFocusTimeoutMs }, ref) {
