...
- **Your Steam wishlist, as a shelf.** Turn on "Enable online features" in the QAM Behavior section (off by default) and you can create a shelf that shows your Steam Store wishlist — automatically synced once a day. You'll see the privacy notice first so you know exactly what Deck Shelves contacts and how often. Two new sort options let you order the wishlist by **Price (low to high)** or **Discount (high to low)**, and three quick templates are added to the shelf picker: "Wishlist", "Wishlist on sale" and "Free wishlist". Wishlist shelves show a small ⚑ flag next to their name in the QAM list so they're easy to spot.
...
- **Switch-Like Home theme — shelves no longer get cut off the screen.** The CSS Loader theme constrains the home page to viewport height with absolute positioning, so DS shelves below the visible area used to be unreachable. The plugin now detects the theme automatically and relaxes the offending layout rules just enough to let gamepad / page scroll reach every shelf — no theme edits required, no impact when the theme is off.
