...
- **Remote Play filter: new "Only on this device" option**, mirroring the existing "Only on another device" — for a shelf of games installed here and nowhere else.
- **Settings are more resilient to data loss.** A field your build didn't yet recognize — from a newer version, another device, or after a downgrade — used to get silently dropped on save; those now round-trip untouched. A corrupted settings file could previously destroy its own backup on the next save, losing both; it's now quarantined and recovered from automatically instead, with a one-time notice if it happens. An in-place update on the standalone host could also leave a stale copy of the plugin running alongside the new one — fixed.
...
- **New, optional toggle: sync your settings across devices on the same Steam account.** Uses your account's own cloud storage (not Steam Cloud, no sign-up, no extra cost) to keep shelves, filters, profiles and every other preference in sync between your Decks/PCs, with a status line showing when it last synced. Off by default, still experimental — if two devices change settings while offline, the most recent change wins.
- **New, optional toggle: Deck Shelves' own idle screensaver.** Instead of Steam's built-in one, show a slideshow of everything your home screen displays — Recents and shelf games — optionally mixing in your local screenshots (shown with the game logo too, when there's one to show) — with your own start delay, time per image, and an optional game-logo overlay you can resize and reposition to any corner. Still experimental. Off by default.
...
- **The project's usage numbers are now public.** The README and the [website](https://santojon.github.io/Deck-Shelves/) show Decky Store installs, GitHub traffic and npm downloads, refreshed weekly — no account or external analytics involved. Larger figures now read compactly (`16.7k` rather than `16,692`).
...
- **New power-user tools (Settings → Advanced).** Clear caches, read a copyable **System information** summary, and turn on **Developer mode** for an on-home debug overlay and the plugin log.
- **Adjustable description text size.** A slider scales the shelf description text from 100 % to 200 %, globally or per shelf.
- **More charts in the Statistics tab.**

### Changed

- **Consistent icons** across the settings pages and the Quick Access panel.
...
- **Quick Search finds more games.** The search now scans every game across every shelf you have on screen — including cards below the fold or still loading their metadata. Accented names match their unaccented spelling (e.g. "café" ↔ "cafe") automatically. If you hit a game whose card isn't mounted yet, the activator scrolls the owning shelf into view and waits for the card to appear before focusing it.
...
- **Online shelves no longer hide games you don't actually own.** On devices with many cloud-play shortcuts (cloud-play catalogues surfaced via Unifideck, etc.), the wishlist / store-on-sale rows were name-matching against those cloud entries and hiding wishlist items that share a title with a game you have access to via subscription but don't own. The name-based dedup now respects the same "include cloud-play games" toggle the appid-based dedup uses, and ignores punctuation differences (so a title written with a colon still matches the same title written without one cleanly).
...
- **Cloud-play sub-toggle for the online shelves** (QAM Online Features + Edit Shelf modal). Cloud-only catalogue entries — like cloud-gaming catalogue entries surfaced via Unifideck — are now kept visible on your wishlist / store-on-sale shelves by default, even with "Include non-Steam shortcuts" on. Locally-installed non-Steam games (from other storefronts) still count as owned. Flip the new sub-toggle on if you want cloud-only entries treated as owned too.
...
- **Artwork falls back through more variants.** Newly released games sometimes ship to the Steam Store before their standard portrait artwork is generated (a few recently-released titles were the prompt). The plugin now tries a wider set of Steam CDN variants (high-resolution portraits, multiple capsule sizes, the landscape header) before falling back to a name placeholder — and the placeholder still shows the discount badge.
...
- **Non-Steam duplicates across launchers preserved.** A game with the same name from two different storefronts is kept as separate entries; only true duplicates are merged.
...
- **UnifiDeck integration** — non-Steam apps managed by UnifiDeck (from other storefronts) surface as sources and tabs in the editor.
