...
- **Menu extraction on SteamOS ≤ 3.7 now hooks the React JSX runtime (`SP_JSX.jsx` / `jsxs`), not just `React.createElement`.** CDP probing on 3.7.21 confirmed the native `onMenuButton` handler emits the `{overview, client}` menu via `(0, s.jsx)(<Component>, {…})`, never via `React.createElement`. The original strategy only patched `React.createElement` — so even though the iteration found the right `onMenuButton` fiber and invoked it, the captured component was always `null` and `showGameMenu` always fell through to the DFL fallback. `extractAppContextMenuLegacy` in `src/core/steamGameMenu.ts` now temporarily wraps `SP_JSX.jsx`, `SP_JSX.jsxs`, and `React.createElement` (defensively, for mixed builds) under a single capture flag while the synthetic `menuFn(fakeEvt)` runs, restoring all three on the `finally` block.
