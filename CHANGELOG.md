...
- **Shelf editor modal footer & content sizing**: the `BottomButtons` save/cancel bar is now compacted via CSS (`padding-top`/`padding-bottom` reduced on the container and buttons). Tab content area bumped from 320 to 360px; the extra room makes the highlight-games mini-preview fit without clipping.
