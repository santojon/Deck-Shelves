...
- **Edit-modal preview consistency.** `ShelfPreview` now caps to `state.limit` (slices the resolved id list before render) and renders trailing cards via the shared decision helper — `MoreCard` and `RefreshCard` no longer appear on every preview regardless of source type. Order is now refresh → more (matches the home shelf).
...
- **Shelf-card context menu now extends the captured native menu (HLTB/cheatdeck pattern).** Previous iteration replaced the captured menu with a DFL-only one for shelf cards, losing the native rich entries (achievements, friends playing, etc.). Restored the discovered-menu path untouched (still validates on SteamOS 3.5–3.9) and now injects the `Deck Shelves > Shelf > […]` submenu via the same seam other plugins use:
  - **Discovered menu**: a wrap-once helper (`getInjectedMenuComponent`) tries `dfl.afterPatch` on `prototype.render` (class), `dfl.afterPatch` on `.render` (forwardRef), or wraps with a HOC (function component). The patched render uses `dfl.findInReactTree` to locate the outermost `Menu` and appends the Deck Shelves submenu to its children. Trigger is the `_dsShelfId` prop the shelf path adds to the captured component's props — non-shelf game cards are unaffected.
  - **DFL fallback menu**: when the legacy fallback path runs (extraction failed) the same `Deck Shelves > Shelf > […]` submenu is appended to the manually-built menu so the user gets the same actions either way.
  - **Both legacy (≤3.7) and modern (3.8+) paths** accept the optional `shelfId` argument to `showGameMenu(appid, shelfId?)`. `shelfCardMenu.ts` (the previous DFL-replacement attempt) was removed.
- **Menu labels now use the bundle's own i18next instance.** `buildDeckShelvesMenuItems` and the DFL-fallback label helper looked up `globalThis.i18next.t` — but our bundle ships its own i18n instance, the host's `globalThis.i18next` doesn't carry our keys, so every label silently fell back to its English string. Switched both call sites to import `i18n` from `../i18n` and use `i18n.t(key)` — labels now translate correctly across all 19 locales (covers the new `menu_deck_shelves` / `menu_shelf` / `collapse_shelf` / `expand_shelf` plus pre-existing `editShelf` / `duplicateShelf` / `move_up` / `move_down` / `hide_shelf` / `show_shelf` / `deleteShelf` / `menu_play` / `menu_properties` / `menu_view_details`).
- **Modal preview — placeholder card height matches game cards.** `PlaceholderCard` (rendered by `GameCard` when no portrait/hero art is available) didn't propagate the `--ds-card-art-h` CSS variable, so `.ds-card-art { height: var(--ds-card-art-h, 100%) }` fell back to the full focusable height (`cardH`, 168px in the preview) while neighbouring game cards used the shorter art height (`artH`, 110px). Result: name-only placeholders looked taller than every other card. Fixed by accepting `artH` on `PlaceholderCard` and forwarding it from `GameCard`'s fallback render path; the home shelf is unchanged because it doesn't pass a separate `artH`.
- **Modal preview — trailing `MoreCard` / `RefreshCard` are no longer focusable.** Both cards added an `interactive: boolean` prop (default `true`, preserves home behavior). `ShelfPreview` now passes `interactive={false}` so the cards render as plain `<div>` instead of `<Focusable>` — gamepad nav skips them entirely (matching the user's expectation that an A press inside a preview shouldn't land on a no-op tile).
