...
- **SavedFilter UI** (Sprint 4.5): EditShelfModal's Filters tab now surfaces a `SavedFiltersBar` at the top — a dropdown to apply a saved filter group to the current shelf, plus a "Save current as filter…" button that prompts for a name and persists the current `filterGroup` as a reusable `SavedFilter`. The QAM grew a collapsible **Saved filters** section (hidden when empty) listing each entry with inline **Rename** (opens a TextField) and **Delete** buttons. Both surfaces consume `settings.savedFilters` and the existing controller actions (`saveFilter` / `renameSavedFilter` / `deleteSavedFilter`) that were already persisted through the Python sanitizer. New i18n keys `saved_filter_apply` / `saved_filter_placeholder` / `saved_filter_save_current` / `saved_filter_save` / `saved_filters_section` / `saved_filter_rename` / `saved_filter_delete` / `saved_filter_empty` landed in en-US and pt-BR; other locales fall back to en-US until translated.
- **DeckRow / ShelfView memoization** (Sprint 5): both components wrapped in `React.memo` with default shallow compare. `rowItems` is already memoized in `ShelfView` via `useMemo`, so unrelated parent re-renders (e.g. toggling an unrelated QAM switch that rebuilds `ShelvesContainer`) no longer cascade into every shelf's full render pass. Low-risk: props are primitive-heavy; the only reference-typed props (`items`, `highlightedAppIds`) are already stable-by-construction from the caller.
