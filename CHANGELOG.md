...
- **Highlight alternating-row patterns**: when the "Highlight specific games" toggle is enabled, two grid buttons appear right below it — **Odd/Even** (highlights the 1st, 3rd, 5th… games) and **Even/Odd** (2nd, 4th, 6th…). Selecting one deselects the other (same `✓`/`·` marker used on the developer filter). Toggling the active pattern button OFF restores the selection snapshot from the moment the pattern was first activated (previously it cleared to empty). Manually toggling any game in the preview below clears both pattern buttons and the snapshot so individual picks are not overwritten.
