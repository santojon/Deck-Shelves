...
- **Online Steam Store integration (opt-in) — `onlineFeaturesEnabled` toggle.** Master toggle (default off) in the QAM Behavior section gates all network-dependent shelf features. First enable shows a privacy banner listing every URL contacted, maximum fetch frequency, and that no PII beyond the public Steam ID is sent. Per-feature sub-toggles `onlineWishlistEnabled` / `onlinePriceSortEnabled` appear beneath the master when enabled. New settings: `onlineFeaturesEnabled`, `onlineWishlistEnabled`, `onlinePriceSortEnabled`, `onlinePrivacyAccepted`. New files: `src/core/onlineStore.ts`.
...
- **Switch-Like Home (CSS Loader theme) coexistence.** The theme locks the home page to viewport height by absolutely-positioning the recents grid + capping its height with `!important` — leaving no native page-scroll path to reach DS shelves below the fold. `ensureStyles` now detects the theme via its `--SLH-lift-hero-px` custom property and tags the documentElement with `data-ds-slh="1"`. Three opt-in CSS rules (gated on the marker) detach the recents grid from absolute positioning and let its inner virtualized container size to natural content, restoring the page flow that lets gamepad / native scroll reach our shelves. The non-themed layout is untouched.
- **Typecheck passes again.** Added `declare const require: any` to `src/globals.d.ts` (covers the dev-only QA harness lazy-require gates), added `"UPDATE"` to `DeckLogScope` (used by `updateNotifier` / `UpdateBanner`), added `updateNotifyEnabled: true` to `defaultSettings()`, the `useSettingsController` initial state, the `resetAll` action, and the test fixture in `src/test/domain/settings.test.ts`. No runtime changes — pure TypeScript reconciliation after the new `Settings` field landed.
