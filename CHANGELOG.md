...
- **`forceCssLoaderThemes` toggle now persists.** The `setForceCssLoaderThemes` controller action was missing (the QAM toggle silently dropped every write), and the backend `_sanitize_settings` whitelist stripped both `forceCssLoaderThemes` and `globalHeroEnabled` on every save. Both fixed — the toggles survive a QAM round-trip and a restart.
...
- **Online shelf context menu title** showed "Shelf" / "Prateleira"; it now shows the game name.
...
- **Shelf action labels disambiguated.** `hide_shelf` / `show_shelf` / `deleteShelf` now include the word "shelf" in every locale (e.g. "Hide shelf", "Show shelf", "Delete shelf" in en-US; "Ocultar prateleira", "Mostrar prateleira", "Apagar prateleira" in pt-BR) to distinguish them from generic Hide / Show / Delete actions in the native game menu.
